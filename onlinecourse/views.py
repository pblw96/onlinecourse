...
def show_exam_result(request, course_id, submission_id):
    ...
    for question in course.question_set.all():
        question_choices = choices.filter(question_id = question.id)
        if question.is_get_score(question_choices):
            score += question.grade
        
        max_score += question.grade

    # Convert the grade to hundreds
    grade = (score * 100) // max_score
